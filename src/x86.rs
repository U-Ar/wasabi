...
const _: () = assert!(size_of::<GeneralRegisterContext>() == 15 * 8);
...
const _: () = assert!(size_of::<InterruptInfo>() == 22 * 8 + 512);
...
enum IdtAttr {
    ...
    // this struct will cause undefined behavior.
    _Notpresent = 0,
...
struct TaskStateSegment64Inner {
    ...
    _ist: [u64; 8], // ist[1..7] (ist[0] is reserved)
    _reserved1: [u16; 5],
...
impl Drop for TaskStateSegment64 {
    fn drop(&mut self) {
        panic!("TSS must never be dropped!");
    }
}
...
/// # Safety
/// Writing to CR3 can cause any exceptions
/// it is programmer's responsibility to setup correct page tables.
#[unsafe(no_mangle)]
...
/// It is programmer's responsibility to ensure no other references exist.
pub unsafe fn take_current_page_table() -> ManuallyDrop<Box<PML4>> {
    unsafe { ManuallyDrop::new(Box::from_raw(read_cr3())) }
}
...
pub unsafe fn put_current_page_table(mut table: ManuallyDrop<Box<PML4>>) {
    ...
    unsafe { write_cr3(Box::into_raw(ManuallyDrop::take(&mut table))) }
}

/// # Safety
/// This function modifies the page table as callback does
/// Anything bad can happen if the page table is corrupted.
