...
impl<T> ConfigRegisters<T> {
    fn read(ecm_base: *mut T, byte_offset: usize) -> Result<T> {
        if !(0..256).contains(&byte_offset) || !byte_offset.is_multiple_of(size_of::<T>()) {
...
impl Pci {
    pub fn new(mcfg: &AcpiMcfgDescriptor) -> Self {
        // To simplify, assume there is one mcfg entry that maps all pci config space
    ...
    pub fn ecm_base<T>(&self, bdf: BusDeviceFunction) -> *mut T {
        (self.ecm_range.start + ((bdf.id as usize) << 12)) as *mut T
    }
    ...
    pub fn write_register_u64(
        ...
        let lo = (data & 0xffff_ffff) as u32;
