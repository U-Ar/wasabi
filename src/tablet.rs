...
fn parse_hid_report_descriptor(report: &[u8]) -> Result<Vec<UsbHidReportInputItem>> {
    ...
    while let Some(prefix) = it.next() {
        ...
        let b_type = match (prefix >> 2) & 0b11 {
            0b00 => UsbHidReportItemType::Main,
            0b01 => UsbHidReportItemType::Global,
            0b10 => UsbHidReportItemType::Local,
            _ => {
...
pub async fn start_usb_tablet(
    ...
    let hid_desc = other_desc_list
        ...
        .next()
        .ok_or("No HID Descriptor found for USB Tablet")?;
