...
impl EfiSystemTable {
    ...
    fn lookup_config_table(&self, guid: &EfiGuid) -> Option<EfiConfigurationTable> {
        for i in 0..self.number_of_table_entries {
            let entry = unsafe { &*self.configuration_table.add(i) };
...
pub fn init_vram(efi_system_table: &EfiSystemTable) -> Result<VramBufferInfo> {
    let graphic_protocol = locate_graphic_protocol(efi_system_table)?;
    let mode = graphic_protocol.mode;
    let info = mode.info;

    Ok(VramBufferInfo {
        buf: mode.frame_buffer_base as *mut u8,
        width: info.horizontal_resolution as i64,
        height: info.vertical_resolution as i64,
        pixels_per_line: info.pixels_per_scan_line as i64,
    })
}
