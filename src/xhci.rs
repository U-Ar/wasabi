...
impl PciXhciDriver {
    ...
    fn setup_xhc_registers(bar0: &BarMem64) -> Result<XhcRegisters> {
        ...
        // number of doorbells = number of slots + 1
        assert!(doorbell_regs.len() == num_slots + 1);
        Ok(XhcRegisters {
            cap_regs,
            op_regs,
            rt_regs,
    ...
    async fn init_port(xhc: &Rc<Controller>, port: usize) -> Result<u8> {
        ...
        info!("xhci: port {port} reset completed.");
        portsc
            .is_enabled()
            .then_some(())
...
impl OperationalRegisters {
    ...
    fn page_size(&self) -> Result<usize> {
        ...
        // bit[n] of page_size_bits is set => PAGE_SIZE = 2^(n+12)
    ...
    fn set_command_bits(&mut self, bits: u32) {
        let c = self.usbcmd.read();
        self.usbcmd.write(c | bits);
    }
...
impl ScratchpadBuffers {
    fn alloc(cap_regs: &CapabilityRegisters, op_regs: &OperationalRegisters) -> Result<Self> {
        ...
        info!(
            "xhci: original num_scratchpad_bufs = {}",
            num_scratchpad_bufs
        );

        let num_scratchpad_bufs = max(cap_regs.num_scratchpad_buffers(), 1);
...
impl EndpointContext {
    ...
    fn set_ring_dequeue_pointer(&mut self, ptr: u64) -> Result<()> {
        self.tr_dequeue_ptr.write_bits(4, 60, ptr >> 4)
    }
    ...
    fn set_deque_cycle_state(&mut self, dcs: bool) -> Result<()> {
...
const _: () = assert!(size_of::<DeviceContext>() == 1024);
...
struct DeviceContextBaseAddressArray {
    ...
    // NB: the index of context is [slot - 1]
...
impl DeviceContextBaseAddressArray {
    ...
    fn set_output_context(&mut self, slot: u8, output_context: Pin<Box<OutputContext>>) {
        ...
        // set it in the actual pointer array
        unsafe {
...
impl Controller {
    ...
    pub fn notify_ep(&self, slot: u8, dci: usize) -> Result<()> {
        let db = self
            ...
            .doorbell_regs
            .get(slot as usize)
            .ok_or("Invalid slot for doorbell")?;
...
impl EventRing {
    ...
    pub fn register_waiter(&mut self, waiter: &Rc<EventWaitInfo>) {
        self.wait_list.push_back(Rc::downgrade(waiter));
    }
}
...
impl TrbRing {
    ...
    fn write(&mut self, index: usize, trb: GenericTrbEntry) -> Result<()> {
        if index < self.trb.len() {
            ...
            Err("TrbRing write index out of range")
        }
    }
    ...
    fn advance_index_notoggle(&mut self, cycle_ours: bool) -> Result<()> {
        if self.current().cycle_state() != cycle_ours {
            return Err("Cannot advance index: cycle state mismatch");
        }
...
impl GenericTrbEntry {
    ...
    fn cmd_result_ok(&self) -> Result<()> {
        if self.trb_type() != TrbType::CommandCompletionEvent as u32 {
            ...
            info!(
                "Completion code was not Success: {}",
                self.completion_code()
            );
            Err("Command did not complete successfully")
        } else {
            Ok(())
        }
    }
    fn transfer_result_ok(&self) -> Result<()> {
        if self.trb_type() != TrbType::TransferEvent as u32 {
            ...
            info!("Transfer failed: {}", self.completion_code());
            Err("Transfer did not complete successfully")
        } else {
            Ok(())
        }
    }
...
// Following From<*Trb> impls are safe since GenericTrbEntry generated from any TRB will be valid
...
impl CommandRing {
    ...
    pub fn push(&mut self, mut src: GenericTrbEntry) -> Result<u64> {
        ...
        if ring.current().cycle_state() != self.cycle_state_ours {
            return Err("CommandRing is full");
        }
        ...
        if ring.current().trb_type() == TrbType::Link as u32 {
            // Reached to Link TRB. Skip it and toggle the cycle state
            ring.advance_index(!self.cycle_state_ours)?;
            self.cycle_state_ours = !self.cycle_state_ours;
        }
        Ok(dst_ptr as u64)
...
impl Doorbell {
    ...
    pub fn notify(&self, target: u8, task: u16) {
        ...
        // SAFETY: This is safe as long as the pointer is valid
        unsafe {
...
impl EventFuture {
    fn new(event_ring: &Mutex<EventRing>, cond: EventWaitCond) -> Self {
        let wait_on = Rc::new(EventWaitInfo {
            cond,
...
impl SetupStageTrb {
    ...
    // 0: Host-to-Device
    // 1: Device-to-Host
    pub const REQ_TYPE_DIR_DEVICE_TO_HOST: u8 = 0b1000_0000;
    pub const REQ_TYPE_DIR_HOST_TO_DEVICE: u8 = 0b0000_0000;
    // bmRequest Type bit[5..=6]: Request Type
    // 0: Standard
    // 1: Class
    // 2: Vendor
    // 3: Reserved
    // pub const REQ_TYPE_TYPE_STANDARD: u8 = 0b0000_0000;
    pub const REQ_TYPE_TYPE_CLASS: u8 = 0b0010_0000;
    pub const REQ_TYPE_TYPE_VENDOR: u8 = 0b0100_0000;
    // bmRequest Type bit[0..=4]: Recipient
    // 0: Device
    // 1: Interface
    // 2: Endpoint
    // 3: Other
    // 4..31: Reserved
    pub const REQ_TYPE_TO_DEVICE: u8 = 0b0000_0000;
    pub const REQ_TYPE_TO_INTERFACE: u8 = 0b0000_0001;
    // pub const REQ_TYPE_TO_ENDPOINT: u8 = 0b0000_0010;
    // pub const REQ_TYPE_TO_OTHER: u8 = 0b0000_0011;
    ...
    pub fn new(request_type: u8, request: u8, value: u16, index: u16, length: u16) -> Self {
        ...
        const TRT_NO_DATA_STAGE: u32 = 0b00;
        const TRT_OUT_DATA_STAGE: u32 = 0b10;
        const TRT_IN_DATA_STAGE: u32 = 0b11;
...
// If there is not data transfer, status direction should be "in"
#[derive(Copy, Clone)]
#[repr(C, align(16))]
