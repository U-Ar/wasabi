...
/// # Safety
/// Implementing this trait is safe only if the target type can be constructed
/// from any byte sequences that has the same size. If not, modification made
/// via the slice produced by as_mut_slice can be an undefined behavior
/// since the bytes cannot be interpreted as the original type
pub unsafe trait IntoPinnedMutableSlice: Sized + Copy + Clone {
    ...
    fn as_mut_slice_sized(self: Pin<&mut Self>, size: usize) -> Result<Pin<&mut [u8]>> {
        if size <= size_of::<Self>() {
            ...
            Err("size exceeds the size of the target type")
        }
    }
}
